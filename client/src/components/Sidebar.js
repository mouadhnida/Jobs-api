...
function Sidebar() {
  const lis = [{name:"Stats", link: "/", icon: <IoStatsChart/> },
               {name:"All Job", link: "/all-jobs", icon: <FaSearch/>}, 
      ...
      {lis.map((li, i) => {
        ...
        <Link to={li.link}>
          <div className="flex items-center w-full h-full pl-10 group-hover/sidebar:bg-slate-100 group-hover/sidebar:pl-14 transition-all duration-500">
            <div className="mr-4 w-6 h-6 group-hover/sidebar:text-main transition-all duration-500">{li.icon}</div>
            <span className="group-hover/sidebar:text-black">{li.name}</span>
          </div>
        </Link>
        </li>)
      }) 
